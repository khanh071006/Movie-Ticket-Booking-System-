...
const navItems = [
    ...
    { name: 'Lịch chiếu', path: '/manager/showtimes', icon: CalendarDays },
    { name: 'Loại ghế', path: '/manager/seat-types', icon: Armchair },
    { name: 'Loại vé', path: '/manager/ticket-types', icon: Ticket },
    { name: 'Đồ ăn', path: '/manager/snacks', icon: Coffee },
];
            ...
            <aside className="hidden w-64 flex-shrink-0 flex-col border-r border-white/5 bg-[#0F0F0F] md:flex h-full">
                ...
                <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-6 custom-scrollbar">
                    {filteredNavItems.map((item) => {
                        const active = location.pathname === item.path || (item.path !== '/manager' && location.pathname.startsWith(item.path));
                        return (
                            <Link
                                key={item.name}
                                to={item.path}
